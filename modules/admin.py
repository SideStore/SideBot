...
class Admin(Cog):
    ...
    async def clean(self, inter: Interaction, count: int, member: Member | None = None):
        ...
        await inter.followup.send(f"Attempted to delete {count} messages, actually deleted {len(del_messages)}!", ephemeral=True)

    @clean.error
    async def app_command_error(self, inter: Interaction, err: errors.AppCommandError):
        if isinstance(err, errors.MissingPermissions):
            return await inter.response.send_message("You do not have permission to run this command.", ephemeral=True)
